...
const ReviewerAdjudication = () => {
  ...
  const handleSubmitReview = (id) => {
    // Submit review logic
    console.log(`Review submitted for ${id} with status: ${reviewStatus[id]} and comment: ${comments[id]}`);
  };

  const getStatusBadge = (status) => {
    switch(status) {
      case 'pending':
        return <span className="status-badge pending"><Clock size={14} /> Pending Review</span>;
      case 'reviewed':
        return <span className="status-badge reviewed"><CheckCircle size={14} /> Reviewed</span>;
      case 'verified':
  ...
  const pendingCount = reviewItems.filter(item => item.status === 'pending' || !reviewStatus[item.id]).length;
    ...
    <div className="reviewer-page">
      <div className="page-header">
        ...
        <div className="header-stats">
          ...
          <div className="stat-item">
            <span className="stat-number">{reviewItems.filter(item => reviewStatus[item.id] === 'accepted' || item.status === 'verified').length}</span>
                ...
                {reviewStatus[item.id] && (
                  <div className="review-result">
                    ...
                    <span>
                      ...
                      {reviewStatus[item.id] === 'modified' && 'Finding requires modification. Please provide updated text.'}
